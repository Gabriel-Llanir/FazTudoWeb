...
export async function create(formData){
    const token = cookies().get("faztudoweb_token")
    const options = {
        method: "POST",
...
export async function apagar(id){
    ...
    if (resp.status !== 204) return {error: "Erro ao apagar publicação. "}
...
export async function update(solicitacao){
    ...
    if (resp.status !== 200) return {error: "Erro ao atualizar publicação"}
